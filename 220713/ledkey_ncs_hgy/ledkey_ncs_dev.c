...
#define   LEDKEY_DEV_NAME            "ledkey_poll"
...
//						key 1 : 커널 타이머 시작
//==================================================================
static int my_timer_init(struct file* filp)
{
	int ret = 0;
...
//                      key 4 : 커널 타이머 종료
//==================================================================
static void my_timer_exit(struct file* filp)
{
	KERNEL_TIMER_MANAGER* ptrmng = (KERNEL_TIMER_MANAGER*)filp->private_data;
...
//	get_user(kbuf, buf);
	...
	led_write(kbuf);
	printk("buf = %lu, kbuf = %lu\n",(unsigned long)buf ,(unsigned long)kbuf);
    return count;
//	return -EFAULT;
}
//=======================================================================
...
static long ledkey_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch( cmd )
    {
		...
		case TIMER_VALUE :
			...
			ptrmng->time_val = info.timer_val;
			break;
        default:
            break;
    }
...
{
	int mask = 0;
	/*WaitQuee_Read 가 timeout 되어 sleep 상태가 된다. 이는 sw를 통해 isr,
	 인터럽트 서비스 루틴이 실행되면 wake up 상태가 된다.*/
	poll_wait(filp, &WaitQueue_Read, wait);
	if(sw_no > 0) mask = POLLIN;
	return mask;
}
