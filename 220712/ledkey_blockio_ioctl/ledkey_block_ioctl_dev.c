...
{
	int i;
	new_jiffies = (unsigned long)get_jiffies_64();
	jiff = new_jiffies - pre_jiffies;
	pre_jiffies = new_jiffies;
	for (i = 0; i < ARRAY_SIZE(key); i++){
		if(irq == sw_irq[i]) {
			sw_no = i+1;
			break;
...
//    printk( "call ioctl -> cmd : %08X, arg : %08X \n", cmd, (unsigned int)arg );
	ioctl_block_info ctrl_info = {0};
	int err, size;
	
	if( _IOC_TYPE( cmd ) != IOCTLBLOCK_MAGIC ) return -EINVAL;
    if( _IOC_NR( cmd ) >= IOCTLBLOCK_MAXNR ) return -EINVAL;
    ...
    switch( cmd )
	{
		case IOCTLBLOCK_INTVAL :
			ctrl_info.intval = jiff;
